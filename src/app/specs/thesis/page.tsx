...
export default function ThesisPage() {
  return (
    ...
    >
      <Section eyebrow="§ 01.1 — the vision" title={<>By 2031, Coconut OS is the default substrate for safe, multi-agent AI compute.</>}>
        <Reveal>
        ...
        <Reveal delay={0.06}>
          <p className="mt-5 max-w-[44rem] text-[15.5px] leading-[1.65] text-[color:var(--fg)]/85">
            The phrase <em className="font-mono not-italic text-[color:var(--accent)]">"I'll just run that agent in a Python process"</em> sounds, in 2031, the way <em className="font-mono not-italic text-[color:var(--muted)]">"I'll just run that web service as root"</em> sounds in 2026.
          </p>
        </Reveal>
      </Section>

      <Section eyebrow="§ 01.2 — the 18-month mission" title={<>Ship Coconut OS 1.0.</>}>
        <Reveal>
          <p className="max-w-[44rem] text-[15.5px] leading-[1.65] text-[color:var(--fg)]/85">
      ...
      <Section eyebrow="§ 01.3 — the lineage" title={<>This is the layer above kvwarden + mlxd.</>}>
        ...
        <Reveal delay={0.05}>
          <div className="mt-9 grid grid-cols-1 md:grid-cols-3 gap-3 max-w-[60rem]">
            <Lineage layer="L2 · CUDA" name="kvwarden" shipped="ships" body="Tenant-fair inference broker on x86/CUDA. Show HN 2026-05-12. Token-bucket admission cut starver tail 29× at A100 + vLLM 0.19.1." />
            <Lineage layer="L2 · MLX"  name="mlxd"     shipped="ships" body="Same thesis on Apple Silicon NPUs. G3 launch 2026-06-09 → 06-16." />
            <Lineage layer="L0 + L1"   name="Coconut OS" shipped="building" body="The OS substrate that turns L2 brokers from products into floor. v1.0 ships 2027 Q4." here />
          </div>
        </Reveal>
        <LockedNote>
...
function Lineage({ layer, name, shipped, body, here = false }: { layer: string; name: string; shipped: "ships" | "building"; body: string; here?: boolean }) {
  return (
    <div className="h-full p-5 rounded-xl border" style={{
      ...
      <div className="mt-1 font-mono text-[10.5px] uppercase tracking-[0.08em]" style={{ color: shipped === "ships" ? "var(--success)" : "var(--highlight)" }}>{shipped === "ships" ? "shipped" : "building — you are here"}</div>
