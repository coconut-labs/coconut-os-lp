...
export default function ThesisPage() {
  return (
    ...
    >
      <Section eyebrow="the vision" title={<>The five-year vision: Coconut OS as the default substrate for safe, multi-agent AI compute.</>}>
        <Reveal>
      ...
      <Section eyebrow="the mission" title={<>Ship Coconut OS 1.0.</>}>
        <Reveal>
          <p className="max-w-[44rem] text-[15.5px] leading-[1.65] text-[color:var(--fg)]/85">
        ...
        <Reveal delay={0.06}>
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-[44rem]">
            <KPI k="boot to first agent" v="≤ 4 min" sub="single-number proxy" />
      ...
      <Section eyebrow="the lineage" title={<>This is the layer above kvwarden + mlxd.</>}>
        <Reveal>
          <p className="max-w-[44rem] text-[15.5px] leading-[1.65] text-[color:var(--fg)]/85">
        ...
        <Reveal delay={0.05}>
          <div className="mt-9 grid grid-cols-1 md:grid-cols-3 gap-3 max-w-[60rem]">
            <Lineage layer="L2 · CUDA" name="kvwarden" body="Tenant-fair inference broker on x86/CUDA. v0.1.6 on PyPI. Token-bucket admission cut the starved tenant's p99 TTFT 26× vs FIFO, from 1,585 ms to 61.5 ms post-warmup, measured on one A100 with vLLM 0.19.1, n=311." />
            <Lineage layer="L2 · MLX"  name="mlxd"     body="The same fairness thesis on Apple Silicon. In research; measured results publish when the repo does." />
            <Lineage layer="L0 + L1"   name="Coconut OS" body="Turns kvwarden and mlxd from infrastructure you assemble into the floor of the machine. v1.0 when the spec survives review." here />
          </div>
        </Reveal>
        <LockedNote>
      ...
      <Section eyebrow="what agent-as-primitive actually means" title={<>An agent is a kernel-recognized execution context.</>}>
        <Reveal>
...
function Lineage({ layer, name, body, here = false }: { layer: string; name: string; body: string; here?: boolean }) {
  return (
