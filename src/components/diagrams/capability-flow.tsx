    ...
    <figure>
      <div className="rounded-2xl border hairline overflow-hidden bg-[color:var(--surface)]/40 p-5 sm:p-7">
        <ol className="relative space-y-3.5">
          {/* spine */}
          <div className="absolute left-[100px] sm:left-[140px] top-3 bottom-3 w-px bg-[color-mix(in_oklab,var(--fg)_15%,transparent)]" aria-hidden />
          {STEPS.map((s, i) => (
            <li key={i} className="grid grid-cols-[100px_24px_1fr] sm:grid-cols-[140px_24px_1fr] gap-2 sm:gap-4 items-start">
              <div className="font-mono text-[11px] text-[color:var(--muted)] tracking-tight pt-1.5 text-right">
                {s.tag}
              </div>
